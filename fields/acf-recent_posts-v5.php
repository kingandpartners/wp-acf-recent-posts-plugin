...
if( !class_exists('acf_field_recent_posts') ) :


class acf_field_recent_posts extends acf_field {
    ...
    */
    ...
    function __construct( $settings )
    {
        ...
        // vars
        $this->name = 'recent_posts';
        $this->label = __("Recent Posts", 'acf');
        ...
        // extra
        add_action('wp_ajax_acf/fields/recent_posts/query', array($this, 'ajax_query'));
        add_action('wp_ajax_nopriv_acf/fields/recent_posts/query', array($this, 'ajax_query'));
    ...
    */
    ...
    function get_ajax_query($options = array())
    {
        ...
        // paged
        $args['posts_per_page'] = $field['list_limit'] ? $field['list_limit'] : 30;
        $args['orderby'] = 'date';
        $args['order'] = 'DESC';
        ...
        // filters
        $args = apply_filters('acf/fields/recent_posts/query', $args, $field, $options['post_id']);
        $args = apply_filters('acf/fields/recent_posts/query/name=' . $field['name'], $args, $field, $options['post_id']);
        $args = apply_filters('acf/fields/recent_posts/query/key=' . $field['key'], $args, $field, $options['post_id']);
    ...
    */
    ...
    function get_post_title($post, $field, $post_id = 0, $is_search = 0)
    {
        ...
        // filters
        $title = apply_filters('acf/fields/recent_posts/result', $title, $post, $field, $post_id);
        $title = apply_filters('acf/fields/recent_posts/result/name=' . $field['_name'], $title, $post, $field, $post_id);
        $title = apply_filters('acf/fields/recent_posts/result/key=' . $field['key'], $title, $post, $field, $post_id);
    ...
    */
    ...
    function render_field($field)
    {
        ...
        $atts = array(
            'id' => $field['id'],
            'class' => "acf-recent_posts {$field['class']}",
            'data-min' => $field['min'],
            'data-max' => $field['max'],
            'data-s' => '',
            'data-post_type' => '',
        ...
        // removing Tags due to overcrowding - not needed for filtering
        unset($term_groups['Tag']);
                    ...
                    <ul class="acf-hl">
                        ...
                        <?php if ($width['taxonomy']): ?>
                            <li style="width:<?php echo $width['taxonomy']; ?>%;">
                                <div class="inner">
                                    <?php if (count($term_groups) > 0): ?>
                                        <select class="filter" data-filter="taxonomy">
                                            <option value=""><?php _e('Filter...', 'acf'); ?></option>
...
//        ));
        ...
        // list_limit
        acf_render_field_setting($field, array(
            'label' => __('Selection list limit', 'acf'),
            'instructions' => __('Maximum number of posts to be displayed on the selection list', 'acf'),
            'type' => 'number',
            'name' => 'list_limit',
            'placeholder' => __(30, 'acf'),
        ));


        // min
        acf_render_field_setting($field, array(
            'label' => __('Minimum number of posts required', 'acf'),
            'instructions' => '',
            'type' => 'number',
            'name' => 'min',
        ));
        ...
        // max
        acf_render_field_setting($field, array(
            'label' => __('Maximum number of posts allowed', 'acf'),
            'instructions' => '',
            'type' => 'number',
            'name' => 'max',
        ));
...
// initialize
new acf_field_recent_posts( $this->settings );
